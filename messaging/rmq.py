    ...
    def connect(self):
        while True:
            try:
                ...
                time.sleep(10)
                LOGGER.info("Retrying to connect to RabbitMQ...")
    ...
    def _wrap_callback(self, callback):
        def wrapped_callback(ch, method, properties, body):
            try:
                ...
                self._handle_failure(body)
        return wrapped_callback

    def _handle_failure(self, body):
        if DLQ_ENABLED:
...
def process_message(message):
    try:
        ...
        index_response = httpx.put(f"{INDEX_SERVICE_ADDRESS}/index/{tag_message['id']}", json={"vectors": vector_ids}, timeout=10.0)
